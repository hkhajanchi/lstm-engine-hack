...
class VectorReverseUnitTester(c: VectorReverse) extends PeekPokeTester(c) {
  ...
  for (i <- 0 until 10) {
    ...
    for (j <- 0 until c.n) {
      ins(j) = rnd.nextInt((1 << c.w-2) - 1)
      poke(c.io.in(j), ins(j))
    }

    for (j <- 0 until c.n) {
      outs(j) = ins(c.n - 1 - j)
      poke(c.io.out(j), outs(j))
    }

    step(1)

    (0 until c.n).foreach( i => expect(c.io.out(i), outs(i)) )
...
/**
  ...
  * From within sbt use:
  * {{{
  * testOnly lstm.VectorReverseTester
  * }}}
  * From a terminal shell use:
  * {{{
  * sbt 'testOnly lstm.VectorReverseTester'
  * }}}
  */
class VectorReverseTester extends ChiselFlatSpec {
  ...
  for ( backendName <- backendNames ) {
    "ReLu" should s"calculate proper greatest common denominator (with $backendName)" in {
      Driver(() => new VectorReverse(8,8), backendName) {
        c => new VectorReverseUnitTester(c)
      } should be (true)
    }
  }

  "Basic test using Driver.execute" should "be used as an alternative way to run specification" in {
    iotesters.Driver.execute(Array(), () => new VectorReverse(8,8)) {
      c => new VectorReverseUnitTester(c)
    } should be (true)
  }
  ...
  if(backendNames.contains("verilator")) {
    "using --backend-name verilator" should "be an alternative way to run using verilator" in {
      iotesters.Driver.execute(Array("--backend-name", "verilator"), () => new VectorReverse(8,8)) {
        c => new VectorReverseUnitTester(c)
      } should be(true)
    }
  }
  ...
  "running with --is-verbose" should "show more about what's going on in your tester" in {
    iotesters.Driver.execute(Array("--is-verbose"), () => new VectorReverse(8,8)) {
      c => new VectorReverseUnitTester(c)
    } should be(true)
  }
